...
train_cat.drop(columns=['Basic_Demos_Enroll_Year'], inplace=True)
...
for col in train_cat.select_dtypes(include='double').columns:
    train_cat[col] = train_cat[col].astype('category')
train_cat.head()
...
test_cat.drop(columns=['Basic_Demos_Enroll_Year'], inplace=True)
#test_cat.head()
...
for col in test_cat.select_dtypes(include='double').columns:
    test_cat[col] = test_cat[col].astype('category')
...
#     if test_cat[col].isna().sum() > 0:
#         test_cat[col] = test_cat[col].fillna("0")  # Avoid inplace test_cat[col].mode()

for col in test_cat.columns:
    if test_cat[col].isna().sum() > 0:
        # Get the most frequent value (mode) for the column
        mode_val = test_cat[col].mode()[0]  # [0] ensures we take the first mode if there are ties
        test_cat[col] = test_cat[col].fillna(mode_val)
...
xgb_classifier = XGBClassifier(objective='binary:logistic', n_estimators=100, learning_rate=0.14, max_depth=5)
